...
class VideoMetadataExtractor:
    """Extracts video and timestamp metadata and saves it to CSV."""

    def __init__(self, video_dir, timestamp_dir, output_file):
    ...
    def extract_video_metadata(self, video_path):
        """Extract basic metadata from a single video file."""
        cap = cv2.VideoCapture(video_path)
    ...
    def extract_timestamp_metadata(self, video_name):
        """Extract phase-related metadata from the corresponding timestamp file."""
        timestamp_file = os.path.join(self.timestamp_dir, f"{video_name}_Clip_List.xlsx")
        ...
        unique_phases = df["Action"].dropna().str.strip().str.lower().unique()
        num_phases = len(unique_phases)
        total_transitions = (df["Action"].shift() != df["Action"]).sum() - 1
    ...
    def extract_data(self):
        """Process all video files and collect metadata."""
        metadata = []
        ...
        for video in os.listdir(self.video_dir):
            if video.endswith((".mkv", ".mp4", ".avi", ".mov", ".flv")):
                ...
                timestamp_info = self.extract_timestamp_metadata(video_name)
    ...
    def save_metadata_as_csv(self, metadata):
        """Save all collected metadata to a CSV file."""
        if not metadata:
            print("No metadata extracted. Skipping file write.")
            return
        ...
        print(f"Video metadata saved to: {self.output_file}")
