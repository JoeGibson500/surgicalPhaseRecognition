...
#     """Normalize phase names by removing variations like (attempt) and (partial)."""
#     phase = phase.strip().lower()
#     phase = re.sub(r"\s*\(attempt\)|\s*\(partial\)", "", phase)  # Removes (attempt) and (partial)
#     return phase

# def get_unique_phases():
#     """Reads all .xlsx files in TIMESTAMP_FOLDER and extracts unique phases."""
#     unique_phases = set()

#     for file in os.listdir(TIMESTAMP_FOLDER):
#         if file.endswith(".xlsx"):
#             file_path = os.path.join(TIMESTAMP_FOLDER, file)
...
#     """Computes the frequency of surgical phases across timestamp files after cleaning names."""
#     all_phases = []
...
#     """Plots a bar chart for surgical phase frequencies with standardized phase names."""
#     phase_counts = compute_phase_frequencies()

#     average_phase_count = np.mean(phase_counts)

#     plt.figure(figsize=(12, 6))
#     sns.barplot(x=phase_counts.index, y=phase_counts.values, color="royalblue", edgecolor="black")
    
#     plt.axhline(average_phase_count, color="red", linestyle="dashed", linewidth=1.5, label=f"Avg Frequency: {average_phase_count:.2f}s")
...
#     """Extracts phase transition data from timestamp files after cleaning names."""
#     phase_durations = {}
...
#                 # Count transitions
#                 phase_sequence = df["Action"].tolist()
#                 num_actions = len(phase_sequence)
...
#                         phase_durations[current_phase].append(next_phase)
#                     else:
#                         phase_durations[current_phase] = [next_phase]

#     return phase_durations


# def compute_phase_transition_matrix():
#     """Computes the phase transition matrix from extracted phase transition data."""
...
#     """Plots a heatmap for surgical phase transitions in a structured order."""
#     transition_matrix, phases = compute_phase_transition_matrix()

#     # Sort phases by frequency to improve structure
#     phase_counts = compute_phase_frequencies()
#     sorted_phases = [phase for phase in phase_counts.index if phase in phases]

#     # Reorder matrix based on sorted phases
...
#     # Plot heatmap with structured ordering
#     plt.figure(figsize=(12, 6))
...
#     plt.xticks(rotation=45, ha="right")
#     plt.yticks(rotation=0)  # Keep Y labels aligned
#     plt.tight_layout()
...
#                 phase_sequence = df["Action"]
#                 phase_start_times = df["Start Time (s)"]
#                 phase_end_times = df["End Time (s)"]
                
#                 num_phases = len(phase_sequence)
                
#                 for i in range(num_phases):
                    
#                     current_phase = phase_sequence[i]
                
#                     phase_duration =  phase_end_times[i] - phase_start_times[i]
                    
#                     if current_phase not in phase_lengths:
#                         phase_lengths[current_phase] = []
#                     phase_lengths[current_phase].append(phase_duration)
...
#     """Plots a bar chart for surgical phase lengths with phases ordered by frequency."""
    
#     # Get average phase lengths
#     phase_lengths = compute_average_phase_length()
    
#     # Get the common sorted phase order from phase frequencies
#     phase_frequencies = compute_phase_frequencies()  
#     sorted_phases = [phase for phase in phase_frequencies.index if phase in phase_lengths]

#     # Ensure phase_lengths aligns with sorted_phases (fill missing phases with NaN)
#     phase_lengths_ordered = [phase_lengths.get(phase, np.nan) for phase in sorted_phases]

#     overall_avg_length = np.mean(phase_lengths_ordered)

#     # Plot using the sorted order
#     plt.figure(figsize=(12, 6))
#     sns.barplot(x=sorted_phases, y=phase_lengths_ordered, color="royalblue", edgecolor="black")
    
#     plt.axhline(overall_avg_length, color="red", linestyle="dashed", linewidth=1.5, label=f"Avg Length: {overall_avg_length:.2f}s")
